...
const globalIdFetcher = (globalId, { db } ) => {
  ...
  switch (type) {
    ...
    case 'Quote':
      return db.collection('qoutes').findOne(ObjectID(id));
    default:
      return null;
  }
};
    ...
    quotesConnection: {
      ...
      resolve: (_, args, { db }) => {
        ...
        return connectionFromPromisedArray(
          db.collection('qoutes').find(findParams).toArray(),
          args
        )
      }
    }
...
const thumbsUpMutation = mutationWithClientMutationId({
  ...
  mutateAndGetPayload: (params, { db }) => {
    ...
    return Promise.resolve(
      db.collection('qoutes').updateOne(
        { _id: ObjectID(id) },
        { $inc: { likesCount: 1 } }
      )
    ).then(result =>
      db.collection('qoutes').findOne(ObjectID(id)));
  }
});
